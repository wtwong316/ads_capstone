...
def get_pie_chart(entered_site):
    if entered_site == 'All':
        ...
        filtered_df2 = filtered_df1.groupby(filtered_df1['Launch Site']).size().reset_index(name='site_count')
        fig = px.pie(filtered_df2, values='site_count',
                     names='Launch Site', title=f'Total Success Launches for All site')
    else:
        # return the outcomes piechart for a selected site
        filtered_df3=spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df4 = filtered_df3.groupby(filtered_df3['class']).size().reset_index(name='class_count')
        fig=px.pie(filtered_df4,values='class_count',names='class',
